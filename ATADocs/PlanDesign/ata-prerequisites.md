...
ATA comprend deux composants : le centre ATA et la passerelle ATA. Pour plus d’informations sur les composants d’ATA, consultez [Architecture d’ATA](/advanced-threat-analytics/Understand/ata-architecture).
...
-   **Contrôleurs de domaine** en cours d’exécution sur Windows Server 2008 et versions ultérieures.
...
L’installation du centre ATA en tant que machine virtuelle est prise en charge. Pour plus d’informations, consultez [Configurer la mise en miroir des ports](configure-port-mirroring.md).
...
>
> - Le centre ATA nécessite au minimum 21 jours de données pour effectuer une analyse comportementale des utilisateurs.
>
> - Pour plus d’informations sur la configuration matérielle requise, consultez [Planification de la capacité ATA](ata-capacity-planning.md).
...
> [!NOTE]
> Les certificats auto-signés ne doivent être utilisés que dans le cadre d’un déploiement en laboratoire.
...
Pour plus d’informations, consultez [Planification de la capacité ATA](ata-capacity-planning.md).
...
## Console ATA
L’accès à la console ATA s’effectue au moyen d’un navigateur configuré comme ceci :
